...
local AttackRangedDarts = radiant.class()

AttackRangedDarts.name = 'attack ranged darts'
AttackRangedDarts.does = 'stonehearth:combat:attack_ranged'
AttackRangedDarts.args = {
   target = Entity
}
AttackRangedDarts.priority = 0
AttackRangedDarts.weight = 1

-- TODO: prohibit attacking at melee range
function AttackRangedDarts:start_thinking(ai, entity, args)
   ...
   self._attack_types = stonehearth.combat:get_combat_actions(entity, 'swamp_goblins:combat:ranged_darts')
...
function AttackRangedDarts:_choose_attack_action(ai, entity, args)
   ...
   self._think_timer = stonehearth.combat:set_timer("AttackRangedDarts waiting for cooldown", 1000, function()
         self._think_timer = nil
         self:_choose_attack_action(ai, entity, args)
      end)
end

function AttackRangedDarts:stop_thinking(ai, entity, args)
...
function AttackRangedDarts:run(ai, entity, args)
...
function AttackRangedDarts:_add_shoot_timer(entity, target, time_to_shoot)
   local shoot_timer = stonehearth.combat:set_timer("AttackRangedDarts shoot", time_to_shoot, function()
      self:_shoot(entity, target, self._weapon_data)
   end)
   table.insert(self._shoot_timers, shoot_timer)
end

function AttackRangedDarts:stop(ai, entity, args)
...
function AttackRangedDarts:_shoot(attacker, target, weapon_data)
   ...
   impact_trace = radiant.events.listen(projectile, 'stonehearth:combat:projectile_impact', function()
         if projectile:is_valid() and target:is_valid() then
            if not assault_context.target_defending then
               ...
               local total_damage = stonehearth.combat:_calculate_dart_damage(attacker, target, attack_info)
...
function AttackRangedDarts:_create_projectile(attacker, target, projectile_speed, projectile_uri)
...
function AttackRangedDarts:_get_world_location(point, entity)
...
function AttackRangedDarts:_get_projectile_offsets(weapon_data, entity)
