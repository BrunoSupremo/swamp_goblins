...
local EDGE_COLOR = Color4(75,74,50, 0)
local FACE_COLOR = Color4(75,74,50, 224)
...
function WaterRenderer:initialize(render_entity, datastore)
   ...
   local selectable = radiant.util.get_config('water.selectable', false)
   stonehearth.selection:set_selectable(self._entity, selectable)
end
...
function WaterRenderer:_update()
   if not stonehearth.subterranean_view:is_ready() then
      -- the trace on stonehearth:visible_volume_changed will trigger the update
      return
   end
   ...
   self._outline_node = _radiant.client.create_region_outline_node(self._parent_node, render_region, EDGE_COLOR, FACE_COLOR, 'materials/water.material.json', 1)
